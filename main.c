...
//const int BITMAP_IMAGE_SIZE = (uint32_t)*sizeof(pixel_t);
const int BITMAP_IMAGE_SIZE = (262144 *sizeof(pixel_t)); //size found from ih.bmp_bytesz


void canny_task(void){
	
	 TickType_t xLastWakeTime;
	 const TickType_t xFrequency = 4000; /* ms */
	 TickType_t functionFpsTest = 0;
	 int8_t run_count = 0;
     // Initialise the xLastWakeTime variable with the current time.
     xLastWakeTime = xTaskGetTickCount();
	 //edgeQueue = xQueueCreate(25, BITMAP_IMAGE_SIZE);
	
	 const char* testImg = "media/lenna.bmp";

	 pixel_t* in_bitmap_data = load_bmp(testImg, &ih);
	 if (in_bitmap_data == NULL) {
		 fprintf(stderr, "main: BMP image not loaded.\n");
		 return 1;
	 }
	 edgeQueue = xQueueCreate(2, ih.bmp_bytesz*sizeof(pixel_t));

	 printf("Info: %d x %d x %d, %d \n", ih.width, ih.height, ih.bitspp, ih.bmp_bytesz);
	 ...
	 if (edgeQueue == NULL)
	{
	 ...
	 for( ;; )
     {
		 ...
		 in_bitmap_data = load_bmp(testImg, &ih);
         // Wait for the next cycle.
         vTaskDelayUntil( &xLastWakeTime, xFrequency );

         // Perform action here.
		 	
		pixel_t *out_bitmap_data = canny_edge_detection(in_bitmap_data, &ih, 40, 80, 1.0);
		...
		if( edgeQueue != 0 )
		{
			if( xQueueSend( edgeQueue,
						 out_bitmap_data,
						( TickType_t ) 1 ) != pdPASS )
			{
				/* Failed to post the message, even after x ticks. */
				fprintf(stderr, "failed to put message into queue.\n");
			}
			else {
				int qsize = uxQueueSpacesAvailable(edgeQueue);
				printf( "put message into queue. no of items = %d\n" , qsize);
			}
		}
		/*
		pixel_t* out_bitmap_data2 = (pixel_t*)malloc(ih.bmp_bytesz * sizeof(pixel_t));
		xQueueReceive(edgeQueue, out_bitmap_data2, 0);
		if (save_bmp("media/out.bmp", &ih, out_bitmap_data2)) {
...
const uint32_t ulLongTime_ms = pdMS_TO_TICKS( 1000UL );
	...
	***NOTE*** Tasks that use the network are created in the network event hook
	...
	//xTaskCreate(helloTask, "test", configMINIMAL_STACK_SIZE, NULL, 1, &HT);
	#ifdef TEST_CANNY
